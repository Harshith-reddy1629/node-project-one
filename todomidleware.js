...
const createTask = async(req,res)=>{
    const { task,status='todo',selected=false,note='Ex: top priority' } = req.body
    const {id}= req.user 
    ...
    try{    
        if (!task ){
            ...
            const taskCreated = await todoData.create({userId:id,task,status,selected})
...
const deleteTask = async(req,res)=>{
    ...
    try{ const deletingTask = await todoData.findByIdAndDelete(id)
        if (!deletingTask){
            res.status(404).send({
